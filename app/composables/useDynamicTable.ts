...
import type { DynamicTableColumn, TableRow, ValueResolver } from '~/types/dynamic-table'

const MIN_COLUMN_WIDTH = 140

type DynamicTableOptions = {
  columns: Ref<DynamicTableColumn[]>
  rows: Ref<TableRow[]>
  storageKey: Ref<string>
  rowKey: Ref<string | ((row: TableRow, index: number) => string | number)>
}

export function useDynamicTable(options: DynamicTableOptions) {
    ...
    try {
      ...
      if (persistedOrder) {
        ...
        if (Array.isArray(parsedOrder)) {
          const validPersistedOrder = parsedOrder.filter((key: unknown): key is string => {
      ...
      if (persistedEnabledColumns) {
        ...
        if (Array.isArray(parsedEnabledColumns)) {
          enabledColumnKeys.value = parsedEnabledColumns.filter((key: unknown): key is string => {
            return typeof key === 'string' && availableColumnKeys.value.includes(key)
          })
        }
      }
  ...
  function toggleSort(column: DynamicTableColumn) {
    if (!column.sortable) {
      return
    }
  ...
  function startColumnResize(event: MouseEvent, columnKey: string) {
    ...
    const currentWidth =
      columnWidths.value[columnKey] ?? headerElements.value[columnKey]?.getBoundingClientRect().width ?? MIN_COLUMN_WIDTH
  ...
  function setHeaderElement(columnKey: string, element: Element | ComponentPublicInstance | null) {
  ...
  function resolveDisplayValue(row: TableRow, column: DynamicTableColumn) {
  ...
  function isFilterActive(value: unknown): boolean {
    ...
    if (typeof value === 'object') {
      return Object.values(value as Record<string, unknown>).some((nestedValue) => isFilterActive(nestedValue))
    }
